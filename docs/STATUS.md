...
| T1 | **Theory-first loop** (`docs/THEORY_FIRST.md`): obligations gated on mechanism and on recomputed `log2|M|` vs `n`, ranked by Jev, compute used only as a falsifier. First instrument `experiments/algebraic_relation.py` reaches N=8192 with a 289-coefficient class where the DFAO route reaches n=64; center indistinguishable from the random null, Thue-Morse recovers its textbook relation and plateaus over six lengths (`docs/experiment-logs/2026-09-18-theory-first-loop-and-algebraic-relations.md`). Next: work the Christol transcendence and unsettled-core obligations in `queue/theory/obligations.json` | Research | Hours | — |
