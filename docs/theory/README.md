...
| Hybrid wedge evaluator for centre bits | **Closed, now with a number.** The wedge supplies 18.5–28.1% of the centre bit's dependency cone and its share *decays* (free-fraction exponent −0.119); work and cone grow at the same rate (2.026 vs 1.990) to T=1024. A constant factor that shrinks, not a growth rate. | `2026-09-15-four-structural-probes.md` |
| Short-circuit pruning of the diagonal recursion | **Closed as a shortcut** — the OR cancellation is exact and saves 14–28% of cells, with the growth exponent unchanged (1.969 vs 1.951 to t=1024). | `2026-09-15-four-structural-probes.md` |
| Window / subshift-of-finite-type invariants excluding the alternating-centre loops | **Closed for width ≤ 15.** The seed's row language is the FULL window set at every `k = 2..15`, so the best invariant true of the seed is true of every row and excludes nothing. Positive control: rule 90 shows 87 of 16,384 windows at k=14 on a matched horizon. Untouched: wider windows, and non-window (counting or algebraic) invariants. | `2026-09-15-four-structural-probes.md` |
| Compact summaries as a zero-word branch state | **Closed for the seven tested forms.** Each is refuted on arbitrary histories with explicit counterexamples (2,509 events, 1,258–2,000 twinned). The seed orbit yields 3 events with no two sharing a summary value, so it has no power to test any of them. | `2026-09-15-four-structural-probes.md` |
