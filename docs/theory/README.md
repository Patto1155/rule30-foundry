...
| Annihilator search over **space-time** patches | **Vacuous by construction** — the local rule is itself a degree-2 relation on a 2-row patch, so the search recovers Rule 30 and its ideal multiples and nothing else. Demonstrated: 6 of 6 rule instances recovered on a 2x8 patch, 0 violations. Reopening it means ideal membership modulo the rule, not a rank computation. `python experiments/algebraic_annihilator.py --space-time 8` | §5 below |
