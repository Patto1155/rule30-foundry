...
## 6. Equal-length loops settle the arbitrary-period strip question

The follow-up searches the synchronized product of each recurrent strip SCC
with itself. Both paths begin at the same full state (phase and row). A Boolean
flag records whether their left-neighbor outputs have differed. Reaching a
common state with that flag set yields two equal-length paths; appending the
same return path closes them at their common root. The supplied verifier checks
the two walks directly with the Wolfram rule-number truth table, independently
of the graph search.

Let their distinct left-output blocks be A and B, both length L. Every infinite
concatenation of the walks is a valid open-strip trajectory. Choose B at block
indices that are powers of two and A otherwise. The selector has infinitely
many ones and unbounded zero gaps, so it is not eventually periodic.
If the output had eventual period P, shifting by LP bits would give equality
of blocks P positions apart. Since A and B differ, the selector would have
eventual period P, a contradiction. Thus the strip admits a non-eventually-
periodic neighbor, not merely a neighbor with a larger period.

Explicit witnesses exist at widths 3, 5, 7, 9, 11, 13, 15 for center word `01`.
Their block lengths are 4, 4, 8, 8, 8, 24, 28. These are seven finite-strip
statements. They imply neither consistent extension to all spatial positions
nor reachability from the single seed.

## 7. The actual exterior condition removes the free choices

Fix a center trace c(t), and fix the complete positive-half initial row at
some time s. Induction in time uniquely determines every positive cell: its
update uses only other positive cells or the prescribed center boundary.
Then center consistency uniquely specifies the required left neighbor:

```
ell(t) = c(t+1) XOR (c(t) OR r_1(t)).
```

Therefore at most one of the two distinct loop output blocks can be compatible
with that complete right initial state. More generally, arbitrary independent
choices between the loops are not licensed. Their right-edge inputs must be
generated by a single common exterior evolution, rather than chosen afresh.
This uniqueness statement does not imply that the remaining output is periodic.

For each saved loop pair, retaining its positive strip cells and setting all
farther-right initial cells to zero rejects both loops within the first block.
That is a check of one finite completion per width, not all completions and not
an actual seed snapshot. Separately, the driven half-line experiment starts
from actual seed right rows at onsets 0..64 but overwrites the center with an
alternating boundary. These are controlled subsystems, not actual seed orbits.
At horizon 2048 all 65 fail to have any suffix period <=64. This prevents an
empirical argument that finite right support causes rapid small-period settling;
it does not establish asymptotic nonperiodicity of the driven subsystem.

The unresolved proof obligation is compatibility with the *left* dynamics from
the same actual seed row. The required ell(t) above must equal that dynamically
generated neighbor for every later time. No inductive invariant establishing
or refuting this for all possible onsets was obtained by these experiments.
